...
function SellerProductOperation() {
  ...
  useEffect(() => {
    if (operation === "edit" && productEditData.location?.coordinates) { 
  ...
  const [formData, setFormData] = useState({
    ...
    location: {
      coordinates: operation === "edit" ? productEditData.location?.coordinates || [long, lat] : [long, lat],
    },
  ...
  useEffect(() => {
    console.log("Updating coordinates in formData:", { lat, long });
  ...
  const handleSubmit = async () => {
    ...
    if (!formData.location.coordinates[1] || !formData.location.coordinates[0] || 
      ...
      notify("Please select location on the map", "info");
      return;
    }
